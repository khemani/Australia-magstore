<?php
require_once(VQMod::modCheck(DIR_SYSTEM . 'library/myocpod.php'));
class Cart {
	private $config;
	private $db;
	private $data = array();
...
$registry->set('myocpod', new Myocpod($registry));
				$this->myocpod = $registry->get('myocpod');
	...
	public function getProducts() {
			...
			foreach ($this->session->data['cart'] as $key => $quantity) {
				...
				if ($product_query->num_rows) {
					...
					{
$option_data = $this->myocpod->getOptionData($option_data, $price, $quantity);
				...
				$option_special = $this->myocpod->getOptionPrice($option_data, $price, $quantity, false);
						...
						'stock'                     => $stock,
						'price'                     => ($product_query->row['price']),
						'total'                     => ($product_query->row['price']) * $quantity,
