...
class ModelCatalogProduct extends Model {
	public function addProduct($data) {
		...
		$this->db->query("INSERT INTO " . DB_PREFIX . "product SET model = '" . $this->db->escape($data['model']) ."', product_type = '" . $this->db->escape($data['product_type']) . "', sku = '" . $this->db->escape($data['sku']) . "', upc = '" . $this->db->escape($data['upc']) . "', ean = '" . $this->db->escape($data['ean']) . "', jan = '" . $this->db->escape($data['jan']) . "', isbn = '" . $this->db->escape($data['isbn']) . "', mpn = '" . $this->db->escape($data['mpn']) . "', periodicity = '" . $this->db->escape($data['periodicity']). "', mlanguage = '" . $this->db->escape($data['mlanguage']) . "', issues_per_year = '" . $this->db->escape($data['issues_per_year']) ."', issn_print = '" . $this->db->escape($data['issn_print']) . "', iisn_online = '" . $this->db->escape($data['iisn_online']) ."', marketed_by = '" . $this->db->escape($data['marketed_by']) ."', supplied_by = '" . $this->db->escape($data['supplied_by']) ."', estimated_delivery = '" . $this->db->escape($data['estimated_delivery']) ."', remarks = '" . $this->db->escape($data['remarks']) . "', note = '" . $this->db->escape($data['note']) . "', origin = '" . $this->db->escape($data['origin']) . "', location = '" . $this->db->escape($data['location']) . "', quantity = '" . (int)$data['quantity'] . "', minimum = '" . (int)$data['minimum'] . "', subtract = '" . (int)$data['subtract'] . "', stock_status_id = '" . (int)$data['stock_status_id'] . "', date_available = '" . $this->db->escape($data['date_available']) . "', manufacturer_id = '" . (int)$data['manufacturer_id'] . "', shipping = '" . (int)$data['shipping'] . "', price = '" . (float)$data['price'] . "', points = '" . (int)$data['points'] . "', weight = '" . (float)$data['weight'] . "', weight_class_id = '" . (int)$data['weight_class_id'] . "', length = '" . (float)$data['length'] . "', width = '" . (float)$data['width'] . "', height = '" . (float)$data['height'] . "', length_class_id = '" . (int)$data['length_class_id'] . "', status = '" . (int)$data['status'] . "', tax_class_id = '" . $this->db->escape($data['tax_class_id']) . "', sort_order = '" . (int)$data['sort_order'] . "', date_added = NOW()");
...
$this->db->query("INSERT INTO " . DB_PREFIX . "myoc_pod_setting SET
					...
					if (isset($product_option['product_option_value']) && count($product_option['product_option_value']) > 0 ) {
						foreach ($product_option['product_option_value'] as $product_option_value) {
							$this->db->query("INSERT INTO " . DB_PREFIX . "product_option_value SET product_option_id = '" . (int)$product_option_id . "', product_id = '" . (int)$product_id . "', option_id = '" . (int)$product_option['option_id'] . "', option_value_id = '" . (int)$product_option_value['option_value_id'] . "', quantity = '" . (int)$product_option_value['quantity'] . "', totalissues = '" . $product_option_value['totalissues'] . "', subtract = '" . (int)$product_option_value['subtract'] . "', price = '" . (float)$product_option_value['price'] . "', price_prefix = '" . $this->db->escape($product_option_value['price_prefix']) . "', points = '" . (int)$product_option_value['points'] . "', points_prefix = '" . $this->db->escape($product_option_value['points_prefix']) . "', weight = '" . (float)$product_option_value['weight'] . "', weight_prefix = '" . $this->db->escape($product_option_value['weight_prefix']) . "'");
if(!isset($product_option_value_id)) {
					$product_option_value_id = $this->db->getLastId();
				} else {
					$temp_povid = $this->db->getLastId();
					$pod_check_query = $this->db->query("SELECT * FROM " . DB_PREFIX . "myoc_pod WHERE product_option_value_id = '" . (int)$product_option_value_id . "'");
					if($pod_check_query->num_rows) {
						$product_option_value_id = $temp_povid;
					} else {
						$pov_check_query = $this->db->query("SELECT * FROM " . DB_PREFIX . "product_option_value WHERE product_option_value_id = '" . (int)$temp_povid . "'");
						if($pov_check_query->num_rows) {
							$product_option_value_id = $temp_povid;
						}
					}
				}//echo 'poddd'.$product_option_value_id;
				if(isset($product_option_value['pod']) && !empty($product_option_value['pod'])) {
					foreach($product_option_value['pod'] as $pod) {
					  $pod['date_start']=  date("Y-m-d", strtotime("-1 days", strtotime(date('Y-m-d H:i:s'))));
					  $pod['date_end']=  date("Y-m-d", strtotime("+5 years", strtotime(date('Y-m-d H:i:s'))));
					
						$this->db->query("INSERT INTO " . DB_PREFIX . "myoc_pod SET 
							product_option_value_id	= '" . (int)$product_option_value_id . "',
							customer_group_id		= '" . (int)$pod['customer_group_id'] . "',
							quantity				= '" . (int)$pod['quantity'] . "',
							option_base_price		= '" . (isset($pod['option_base_price']) && $pod['option_base_price'] ? 1 : 0) . "',
...
$this->db->query("INSERT INTO " . DB_PREFIX . "myoc_pod_setting SET
					...
					if (isset($product_option['product_option_value'])  && count($product_option['product_option_value']) > 0 ) {
						foreach ($product_option['product_option_value'] as $product_option_value) {
							$this->db->query("INSERT INTO " . DB_PREFIX . "product_option_value SET product_option_value_id = '" . (int)$product_option_value['product_option_value_id'] . "', product_option_id = '" . (int)$product_option_id . "', product_id = '" . (int)$product_id . "', option_id = '" . (int)$product_option['option_id'] . "', option_value_id = '" . (int)$product_option_value['option_value_id'] . "', quantity = '" . (int)$product_option_value['quantity'] . "', totalissues = '" . $product_option_value['totalissues'] . "', subtract = '" . (int)$product_option_value['subtract'] . "', price = '" . (float)$product_option_value['price'] . "', price_prefix = '" . $this->db->escape($product_option_value['price_prefix']) . "', points = '" . (int)$product_option_value['points'] . "', points_prefix = '" . $this->db->escape($product_option_value['points_prefix']) . "', weight = '" . (float)$product_option_value['weight'] . "', weight_prefix = '" . $this->db->escape($product_option_value['weight_prefix']) . "'");
if(!isset($product_option_value_id)) {
					$product_option_value_id = $this->db->getLastId();
				} else {
					$temp_povid = $this->db->getLastId();
