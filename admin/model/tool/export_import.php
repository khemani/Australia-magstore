...
// extract the product details
        ...
        $periodicity = $this->db->escape($product['periodicity']);
        $issues_per_year = $this->db->escape($product['issues_per_year']);
        $issn_print = $this->db->escape($product['issn_print']);
        $iisn_online = $this->db->escape($product['iisn_online']);
        $marketed_by = $this->db->escape($product['marketed_by']);
        $supplied_by = $this->db->escape($product['supplied_by']);
        $estimated_delivery = $this->db->escape($product['estimated_delivery']);
...
// generate and execute SQL for inserting the product
        $sql = "INSERT INTO `" . DB_PREFIX . "product` (`product_id`,`product_type`,`quantity`,`sku`,`upc`,";
        ...
        $sql .= "`periodicity`,`issues_per_year`,`issn_print`,`iisn_online`,`marketed_by`,`supplied_by`,`estimated_delivery`,`remarks`,`note`,`location`,`stock_status_id`,`model`,`manufacturer_id`,`image`,`shipping`,`price`,`points`,`date_added`,`date_modified`,`date_available`,`weight`,`weight_class_id`,`status`,";
        $sql .= "`tax_class_id`,`viewed`,`length`,`width`,`height`,`length_class_id`,`sort_order`,`subtract`,`minimum`) VALUES ";
        $sql .= "($product_id,$product_type,$quantity,'$sku','$upc',";
        $sql .= in_array('ean', $product_fields) ? "'$ean'," : "";
        $sql .= in_array('jan', $product_fields) ? "'$jan'," : "";
        $sql .= in_array('isbn', $product_fields) ? "'$isbn'," : "";
        $sql .= in_array('mpn', $product_fields) ? "'$mpn'," : "";
        $sql .= "'$periodicity','$issues_per_year','$issn_print','$iisn_online','$marketed_by','$supplied_by','$estimated_delivery','$remarks','$note','$location',$stock_status_id,'$model',$manufacturer_id,'$image',$shipping,$price,$points,";
        $sql .= ($date_added == 'NOW()') ? "$date_added," : "'$date_added',";
        $sql .= ($date_modified == 'NOW()') ? "$date_modified," : "'$date_modified',";
        ...
        if (count($categories) > 0) {
            ...
            foreach ($categories as $category_id) {
                ...
                $sqlcat = "SELECT c.category_id FROM `" . DB_PREFIX . "category` c";
                $sqlcat .= " JOIN `" . DB_PREFIX . "category_description` cd ON cd.category_id=c.category_id ";
                $sqlcat .= " WHERE LOWER(TRIM( REPLACE(REPLACE(cd.name, ' ',''), '&amp;','&'))) LIKE '%$category_id%' ";
                $querycat = $this->db->query($sqlcat);
                $catrow = $querycat->rows;

                $category_id = $catrow[0]['category_id'];
...
// load the worksheet cells and store them to the database
        ...
        for ($i = 0; $i < $k; $i+=1) {
            ...
            $product_type = trim($this->getCell($data, $i, $j++));
            ...
            $periodicity = $this->getCell($data, $i, $j++, '');
            $issues_per_year = $this->getCell($data, $i, $j++, '');
            $issn_print = $this->getCell($data, $i, $j++, '');
            $iisn_online = $this->getCell($data, $i, $j++, '');
            $marketed_by = $this->getCell($data, $i, $j++, '');
            $supplied_by = $this->getCell($data, $i, $j++, '');
            $estimated_delivery = $this->getCell($data, $i, $j++, '');
            $remarks = $this->getCell($data, $i, $j++, '');
            ...
            $product['issues_per_year'] = $issues_per_year;
            $product['issn_print'] = $issn_print;
            $product['iisn_online'] = $iisn_online;
            $product['marketed_by'] = $marketed_by;
            $product['supplied_by'] = $supplied_by;
            $product['estimated_delivery'] = $estimated_delivery;
...
// load the worksheet cells and store them to the database
    ...
    protected function storeProductOptionValueIntoDatabase(&$product_option_value, &$old_product_option_value_ids) {
        ...
        $totalissues = $product_option_value['totalissues'];
        $subtract = $product_option_value['subtract'];
        $subtract = ((strtoupper($subtract) == "TRUE") || (strtoupper($subtract) == "YES") || (strtoupper($subtract) == "ENABLED")) ? 1 : 0;
        $price = $product_option_value['price'];
        $price_prefix = $product_option_value['price_prefix'];
        /**
         *  Inserting the records in Subscription Tables 
         */
        $subs_price = $product_option_value['subs_price'];
        $subs_price_prefix = $product_option_value['subs_price_prefix'];
        $subs_special = $product_option_value['subs_special'];
        $subs_special_prefix = $product_option_value['subs_special_prefix'];
        $subs_customer_group_id = $product_option_value['subs_customer_group_id'];
        $subs_quantity = $product_option_value['subs_quantity'];
        $subs_option_base_price = $product_option_value['subs_option_base_price'];
        /*
         * End 
         */
        ...
        if (isset($old_product_option_value_ids[$product_id][$option_id][$option_value_id])) {
            ...
            $sql .= "(`product_option_value_id`,`product_option_id`,`product_id`,`option_id`,`option_value_id`,`quantity`,`totalissues`,`subtract`,`price`,`price_prefix`,`points`,`points_prefix`,`weight`,`weight_prefix` ) VALUES ";
            $sql .= "($product_option_value_id,$product_option_id,$product_id,$option_id,$option_value_id,$quantity,$totalissues,$subtract,$price,'$price_prefix',$points,'$points_prefix',$weight,'$weight_prefix')";
            $this->db->query($sql);

            /**
             *  Inserting the records in Subscription Tables 
             */
            $sql = "DELETE FROM `" . DB_PREFIX . "myoc_pod` WHERE product_option_value_id = '" . (int) $product_option_value_id . "'";
            $this->db->query($sql);
            $sql = "INSERT INTO `" . DB_PREFIX . "myoc_pod` ";
            $sql .= "(`product_option_value_id`, `customer_group_id`, `quantity`, `option_base_price`, `price`, `price_prefix`, `special`, `special_prefix` ) VALUES ";
            $sql .= "('$product_option_value_id', '$subs_customer_group_id', '$subs_quantity', '$subs_option_base_price', '$subs_price', '$subs_price_prefix', '$subs_special', '$subs_special_prefix')";
            $this->db->query($sql);
            $sql = "DELETE FROM `" . DB_PREFIX . "myoc_pod_setting` WHERE product_option_id = '" . (int) $product_option_id . "'";
            $this->db->query($sql);
            $sql = "INSERT INTO `" . DB_PREFIX . "myoc_pod_setting` ";
            $sql .= "(`product_option_id`, `show_price_product`, `show_price_cart`, `show_final`, `flat_rate`, `table_style`, `price_format`, `qty_column`, `qty_format`, `stock_column`, `qty_cart` ) VALUES ";
            $sql .= "('$product_option_id', '1', '1', '1', '0', 'h', 'total', '0', 'single', '0', '1')";
            $this->db->query($sql);
            //  End
            ...
            $sql = "INSERT INTO `" . DB_PREFIX . "product_option_value` ";
            $sql .= "(`product_option_id`, `product_id`, `option_id`, `option_value_id`, `quantity`, `totalissues`, `subtract`, `price`, `price_prefix`, `points`, `points_prefix`, `weight`, `weight_prefix` ) VALUES ";
            $sql .= "($product_option_id, $product_id, $option_id, $option_value_id, $quantity, $totalissues, $subtract, $price, '$price_prefix', $points, '$points_prefix', $weight, '$weight_prefix')";
            $this->db->query($sql);
    ...
    protected function deleteProductOptionValues() {
        ...
        $this->db->query($sql);
        /**
         * #pk Truncate subscroiption table 
         */
        $sql = "TRUNCATE TABLE `" . DB_PREFIX . "myoc_pod`";
        $this->db->query($sql);

        $sql = "TRUNCATE TABLE `" . DB_PREFIX . "myoc_pod_setting`";
        $this->db->query($sql);
        /**
         * End 
         */
    ...
    protected function deleteProductOptionValue($product_id) {
        ...
        $this->db->query($sql);
        /**
         *  Delete record from Subscription table
         */
        $sql = "DELETE FROM `" . DB_PREFIX . "myoc_pod_setting` WHERE product_option_id in (select product_option_id from `" . DB_PREFIX . "product_option_value` where product_id = '" . (int) $product_id . "') ";
        $this->db->query($sql);
        $sql = "DELETE FROM `" . DB_PREFIX . "myoc_pod` WHERE product_option_value_id in (select product_option_value_id from `" . DB_PREFIX . "product_option_value` where product_id = '" . (int) $product_id . "') ";
        $this->db->query($sql);
        /**
         * End 
         */
    ...
    protected function uploadProductOptionValues(&$reader, $incremental, &$available_product_ids) {
        ...
        for ($i = 0; $i < $k; $i+=1) {
            ...
            $quantity = $this->getCell($data, $i, $j++, '0');
            $totalissues = $this->getCell($data, $i, $j++, '0');
             ...
             *  subscritpion values
             */
            $subs_price = $this->getCell($data, $i, $j++, '0');
            $subs_price_prefix = $this->getCell($data, $i, $j++, '+');
            $subs_special = $this->getCell($data, $i, $j++, '0');
            $subs_special_prefix = $this->getCell($data, $i, $j++, '+');
            $subs_customer_group_id = $this->getCell($data, $i, $j++, '1');
            $subs_quantity = $this->getCell($data, $i, $j++, '0');
            $subs_option_base_price = $this->getCell($data, $i, $j++, '0');
            /**
             * End  
             */
            $points = $this->getCell($data, $i, $j++, '0');
            ...
            $product_option_value['totalissues'] = $totalissues;
             ...
             * subscritpion values
             */
            $product_option_value['subs_price'] = $subs_price;
            $product_option_value['subs_price_prefix'] = $subs_price_prefix;
            $product_option_value['subs_special'] = $subs_special;
            $product_option_value['subs_special_prefix'] = $subs_special_prefix;
            $product_option_value['subs_customer_group_id'] = $subs_customer_group_id;
            $product_option_value['subs_quantity'] = $subs_quantity;
            $product_option_value['subs_option_base_price'] = $subs_option_base_price;
            /**
             * End  
             */
...
";
    ...
    protected function validateProducts(&$reader) {
        ...
        $expected_heading = array
            ("product_id", "product_type", "name", "categories", "sku", "upc",);
        if (in_array("ean", $product_fields)) {
        ...
        $expected_heading = array_merge($expected_heading, array("periodicity", "issues_per_year", "issn_print", "iisn_online", "marketed_by", "supplied_by", "estimated_delivery", "remarks", "note", "location", "quantity", "model", "manufacturer", "image_name", "shipping", "price", "points", "date_added", "date_modified", "date_available", "weight", "weight_unit", "length", "width", "height", "length_unit", "status", "tax_class_id", "seo_keyword", "description"));
        if ($exist_meta_title) {
    ...
    protected function validateProductOptionValues(&$reader) {
        ...
        if ($this->config->get('export_import_settings_use_option_id')) {
            if ($this->config->get('export_import_settings_use_option_value_id')) {
                $expected_heading = array("product_id", "option_id", "option_value_id", "quantity", "totalissues", "subtract", "price", "price_prefix", "subs_price", "subs_price_prefix", "subs_special", "subs_special_prefix", "subs_customer_group_id", "subs_quantity", "subs_option_base_price", "points", "points_prefix", "weight", "weight_prefix");
            } else {
                $expected_heading = array("product_id", "option_id", "option_value", "quantity", "totalissues", "subtract", "price", "price_prefix", "subs_price", "subs_price_prefix", "subs_special", "subs_special_prefix", "subs_customer_group_id", "subs_quantity", "subs_option_base_price", "points", "points_prefix", "weight", "weight_prefix");
            }
        } else {
            if ($this->config->get('export_import_settings_use_option_value_id')) {
                $expected_heading = array("product_id", "option", "option_value_id", "quantity", "totalissues", "subtract", "price", "price_prefix", "subs_price", "subs_price_prefix", "subs_special", "subs_special_prefix", "subs_customer_group_id", "subs_quantity", "subs_option_base_price", "points", "points_prefix", "weight", "weight_prefix");
            } else {
                $expected_heading = array("product_id", "option", "option_value", "quantity", "totalissues", "subtract", "price", "price_prefix", "subs_price", "subs_price_prefix", "subs_special", "subs_special_prefix", "subs_customer_group_id", "subs_quantity", "subs_option_base_price", "points", "points_prefix", "weight", "weight_prefix");
            }
        }
        $expected_multilingual = array();
        return $this->validateHeading($data, $expected_heading, $expected_multilingual);
    ...
    protected function validateProductSubscriptionOptionValues(&$reader) {
        $data = $reader->getSheetByName('ProductSubscriptionOptionValues');
        if ($data == null) {
            return true;
        }
        if ($this->config->get('export_import_settings_use_subscription_option_id')) {
            if ($this->config->get('export_import_settings_use_subscription_option_value_id')) {
                $expected_heading = array("product_option_value_id", "customer_group_id", "quantity", "option_base_price", "price", "price_prefix", "special", "special_prefix");
            } else {
                $expected_heading = array("product_option_value_id", "customer_group_id", "quantity", "option_base_price", "price", "price_prefix", "special", "special_prefix");
            }
        } else {
...
";
    ...
    protected function getProducts(&$languages, $default_language_id, $product_fields, $exist_meta_title, $offset = null, $rows = null, $min_id = null, $max_id = null) {
        ...
        $sql .= "  p.product_id,";
        $sql .= "  p.product_type,";
        $sql .= "  GROUP_CONCAT( DISTINCT CAST(REPLACE(cd.name, '&amp;','&') AS CHAR(255)) SEPARATOR \",\" ) AS categories,";
        $sql .= "  p.sku,";
        $sql .= "  p.upc,";
        if (in_array('ean', $product_fields)) {
            $sql .= "  p.ean,";
        ...
        $sql .= "  p.periodicity,";
        $sql .= "  p.issues_per_year,";
        $sql .= "  p.issn_print,";
        $sql .= "  p.iisn_online,";
        $sql .= "  p.marketed_by,";
        $sql .= "  p.supplied_by,";
        $sql .= "  p.estimated_delivery,";
        $sql .= "  p.remarks,";
        $sql .= "  p.note,";
        ...
        $sql .= "LEFT JOIN `" . DB_PREFIX . "category_description`  cd ON cd.category_id=pc.category_id ";
...
// Set the column widths
        ...
        $worksheet->getColumnDimensionByColumn($j++)->setWidth(max(strlen('product_type'), 4) + 1);
        foreach ($languages as $language) {
            $worksheet->getColumnDimensionByColumn($j++)->setWidth(max(strlen('name') + 4, 30) + 1);
        }
        ...
        if (in_array('mpn', $product_fields)) {
            ...
            $worksheet->getColumnDimensionByColumn($j++)->setWidth(max(strlen('periodicity'), 10) + 1);
            $worksheet->getColumnDimensionByColumn($j++)->setWidth(max(strlen('issues_per_year'), 10) + 1);
            $worksheet->getColumnDimensionByColumn($j++)->setWidth(max(strlen('issn_print'), 10) + 1);
            $worksheet->getColumnDimensionByColumn($j++)->setWidth(max(strlen('iisn_online'), 10) + 1);
            $worksheet->getColumnDimensionByColumn($j++)->setWidth(max(strlen('marketed_by'), 10) + 1);
            $worksheet->getColumnDimensionByColumn($j++)->setWidth(max(strlen('supplied_by'), 10) + 1);
            $worksheet->getColumnDimensionByColumn($j++)->setWidth(max(strlen('estimated_delivery'), 10) + 1);
            $worksheet->getColumnDimensionByColumn($j++)->setWidth(max(strlen('remarks'), 10) + 1);
            $worksheet->getColumnDimensionByColumn($j++)->setWidth(max(strlen('note'), 10) + 1);
        }
...
// The product headings row and column styles
        ...
        $data[$j++] = 'periodicity';
        $data[$j++] = 'issues_per_year';
        $data[$j++] = 'issn_print';
        $data[$j++] = 'iisn_online';
        $data[$j++] = 'marketed_by';
        $data[$j++] = 'supplied_by';
        $data[$j++] = 'estimated_delivery';
        $data[$j++] = 'remarks';
        $data[$j++] = 'note';
...
// The actual products data
        ...
        foreach ($products as $row) {
            ...
            $data[$j++] = $row['periodicity'];
            $data[$j++] = $row['issues_per_year'];
            $data[$j++] = $row['issn_print'];
            $data[$j++] = $row['iisn_online'];
            $data[$j++] = $row['marketed_by'];
            $data[$j++] = $row['supplied_by'];
            $data[$j++] = $row['estimated_delivery'];
            $data[$j++] = $row['remarks'];
            $data[$j++] = $row['note'];
...
// The actual product options data
    ...
    protected function getProductOptionValues($min_id, $max_id) {
        $language_id = $this->getDefaultLanguageId();
        $sql = "SELECT ";
        $sql .= "  p.product_id, pov.option_id, pov.option_value_id, pov.quantity, pov.totalissues, pov.subtract, od.name AS `option`, ovd.name AS option_value,pov.price, pov.price_prefix, ";
        $sql .= " pov.product_option_value_id , pov.product_option_id, ";
        $sql .= " subs.price as `subs_price`, subs.price_prefix as `subs_price_prefix`, subs.special as `subs_special`, subs.special_prefix as `subs_special_prefix` , 
                 subs.customer_group_id as `subs_customer_group_id`, subs.quantity as `subs_quantity`, subs.option_base_price as `subs_option_base_price`, ";
        $sql .= "   pov.points, pov.points_prefix, pov.weight, pov.weight_prefix  ";
        $sql .= "FROM ";
        $sql .= "( SELECT product_id ";
        $sql .= "  FROM `" . DB_PREFIX . "product` ";
        if (isset($min_id) && isset($max_id)) {
        ...
        $sql .= "INNER JOIN `" . DB_PREFIX . "myoc_pod` subs ON subs.product_option_value_id = pov.product_option_value_id ";
...
// Set the column widths
        ...
        $worksheet->getColumnDimensionByColumn($j++)->setWidth(max(strlen('quantity'), 4) + 1);
        $worksheet->getColumnDimensionByColumn($j++)->setWidth(max(strlen('totalissues'), 4) + 1);
        ...
        $worksheet->getColumnDimensionByColumn($j++)->setWidth(max(strlen('subs_price'), 10) + 1, $price_format);
        $worksheet->getColumnDimensionByColumn($j++)->setWidth(max(strlen('subs_price_prefix'), 5) + 1, $text_format);
        $worksheet->getColumnDimensionByColumn($j++)->setWidth(max(strlen('subs_special'), 10) + 1, $price_format);
        $worksheet->getColumnDimensionByColumn($j++)->setWidth(max(strlen('subs_special_prefix'), 5) + 1, $text_format);
        $worksheet->getColumnDimensionByColumn($j++)->setWidth(max(strlen('subs_customer_group_id'), 4) + 1);
        $worksheet->getColumnDimensionByColumn($j++)->setWidth(max(strlen('subs_quantity'), 4) + 1);
        $worksheet->getColumnDimensionByColumn($j++)->setWidth(max(strlen('subs_option_base_price'), 4) + 1);
...
// The heading row and column styles
        ...
        $data[$j++] = 'subs_price';
        $data[$j++] = 'subs_price_prefix';
        $data[$j++] = 'subs_special';
        $data[$j++] = 'subs_special_prefix';
        $data[$j++] = 'subs_customer_group_id';
        $data[$j++] = 'subs_quantity';
        $data[$j++] = 'subs_option_base_price';
...
// The actual product option values data
        ...
        foreach ($product_option_values as $row) {
            ...
            $data[$j++] = $row['quantity'];
            $data[$j++] = $row['totalissues'];
            $data[$j++] = ($row['subtract'] == 0) ? 'false' : 'true';
            $data[$j++] = $row['price'];
            $data[$j++] = $row['price_prefix'];

            $data[$j++] = $row['subs_price'];
            $data[$j++] = $row['subs_price_prefix'];
            $data[$j++] = $row['subs_special'];
            $data[$j++] = $row['subs_special_prefix'];
            $data[$j++] = $row['subs_customer_group_id'];
            $data[$j++] = $row['subs_quantity'];
            $data[$j++] = $row['subs_option_base_price'];
